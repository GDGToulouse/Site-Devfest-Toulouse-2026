...
| `SEED_SCRIPT` | non | **Build arg** (pas une variable runtime) : seed rejoué **à chaque boot** du backend. Voir l'avertissement ci-dessous. Défaut du Dockerfile : `prisma/seed.ts`. | `prisma/seed.ts` |
...
| Script | Comportement | À utiliser sur |
|--------|--------------|----------------|
| `prisma/seed.ts` | **Idempotent** (`upsert`). N'efface rien. | **prod**, **beta** — partout où les données sont saisies à la main |
| `prisma/seed-dev.ts` | **Destructif** : `deleteMany` sur speakers, sponsors et catégories, puis recrée un jeu de démo (3 speakers, 28 sponsors fictifs). | `dev-j` et le local uniquement — environnements jetables |

**Conséquence concrète** : sur un environnement configuré avec `seed-dev.ts`, tout speaker ou sponsor saisi à la main (ou importé depuis Sessionize) est **effacé au prochain déploiement**. C'est voulu pour un bac à sable, et destructeur partout ailleurs.

La bêta doit donc porter `SEED_SCRIPT=prisma/seed.ts` dans ses build args Coolify (case « Available at Buildtime » cochée). `docker-compose.dev.yml` conserve `seed-dev.ts` par défaut pour `dev-j`.
